...
from MinLlama.HelperCode.base_llama import LlamaPreTrainedModel, LlamaConfig
from MinLlama.RoPE.task import apply_rotary_emb
from MinLlama.HelperCode.utils import *


# Root Mean Square Layer Normalization (https://arxiv.org/abs/1910.07467)
# Normalização de Camada por Raiz Quadrada Média (https://arxiv.org/abs/1910.07467)
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        """
        Compute Scaled Dot Product Attention (Section 3.2.1 in https://arxiv.org/abs/1706.03762).
        Args:
        ...
        Calcular a Atenção por Produto Escalar Escalonado (Seção 3.2.1 em https://arxiv.org/abs/1706.03762).
        Argumentos:
            consulta: (bs, n_local_heads, seqlen, head_dim)
            chave: (bs, n_local_heads, seqlen, head_dim)
            valor: (bs, n_local_heads, seqlen, head_dim)
        Retorno:
            saída: (bs, n_local_heads, seqlen, head_dim)
        """
        # Step 1: Compute attention scores (Q @ K^T) / sqrt(d_k)
        # Etapa 1: Calcular as pontuações de atenção (Q @ K^T) / sqrt(d_k)
        scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(self.head_dim)

        # Step 2: Apply softmax to get attention weights
        # Etapa 2: Aplique softmax para obter os pesos de atenção
        attention_weights = F.softmax(scores, dim=-1)

        # Step 3: Apply dropout to attention weights
        # Etapa 3: Aplicar dropout aos pesos de atenção
        attention_weights = self.attn_dropout(attention_weights)

        # Step 4: Apply attention weights to values
       # Etapa 4: Aplicar pesos de atenção aos valores
    ...
    def forward(self, x: torch.Tensor):
        ...
        # restaurar tempo como dimensão de lote e cabeças de concatenação
        output = output.transpose(1, 2).contiguous().view(batch_size, seqlen, -1)

        # final projection into the residual stream
        # projeção final no fluxo residual
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        Passagem direta do bloco Transformer:
        1) Norma da camada + Autoatenção + Conexão residual
        2) Norma da camada + Feedforward + Conexão residual
        """
        # Self-attention block with residual connection
        # Bloqueio de autoatenção com conexão residual
        residual = x
        ...
        # Bloco de feedforward com conexão residual
        residual = x
        x = self.ffn_norm(x)
...
class Llama(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        ...
        # weight tying # amarração de peso
        self.tok_embeddings.weight = self.output.weight

        # init all weights # inicializar todos os pesos
        self.apply(self._init_weights)
        # apply special scaled init to the residual projections, per GPT-2 paper
        # Aplicar inicialização escalonada especial às projeções residuais, conforme o artigo do GPT-2
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if targets is not None:
            ...
            # tempo-de-inferência: encaminhar a saída apenas na última posição
...
#         RMSNorm normaliza cada vetor pela raiz da média dos quadrados:
#         """
#         output = self._norm(x.float()).type_as(x)
#         return output * self.weight
#
...
#         """
#         pass # TODO: implement query_key_value scores computation
#
#     def forward(
#         self,
#         x: torch.Tensor
...
#         """
#         pass # TODO: implement forward pass for Llama
#
# class Llama(LlamaPreTrainedModel):
#     def __init__(self, config: LlamaConfig):
#         """
...
#
#         # some useful precomputing for the RoPE relative positional embeddings
#
#         # init all weights
#         self.apply(self._init_weights)
#         # apply special scaled init to the residual projections, per GPT-2 paper
...
#                 probabilities = torch.nn.functional.softmax(logits, dim=-1)
#                 idx_next = torch.multinomial(probabilities, 1)
#             # append a sampled index to the running sequence and continue
#             idx = torch.cat((idx, idx_next), dim=1)
#
#         return idx
#
...
#   device = 'cuda' if torch.cuda.is_available() else \
#       'mps' if torch.backends.mps.is_available() else \
#       'cpu' # examples: 'cpu', 'cuda', 'cuda:0', 'cuda:1', 'mps', etc.
